...
Also you can just ask Makusu2#2222 cuz they're never too busy to make a new friend <3
    ...
    async def eval(self, ctx, *, to_eval:str):
        ...
        try:
            ...
            logging.error(f"Couldn't get a match on {ctx.message.content}. Weird.")
    ...
    async def reloadcritical(self, ctx):
        ...
        try:
            ...
            await commandutil.send_formatted_message(ctx, commandutil.get_formatted_traceback(caught_exception))
    ...
    async def on_command_error(self, ctx, caught_exception:discord.ext.commands.errors.CommandError):
        if isinstance(caught_exception,discord.ext.commands.errors.CommandNotFound):
        ...
        elif isinstance(caught_exception, discord.ext.commands.errors.NotOwner):
            await ctx.send("Sorry, only Maku can use that command :(")
        elif isinstance(caught_exception, discord.ext.commands.errors.CommandOnCooldown):
            await ctx.send("Slow down! You're going too fast for me ;a;\nI'm sorry that I'm not good enough to keep up with you :(")
        elif isinstance(caught_exception, (discord.ext.commands.errors.MissingPermissions, discord.ext.commands.errors.BotMissingPermissions, discord.ext.commands.errors.BadUnionArgument)):
            await ctx.send(str(caught_exception)) 
        else:
            await commandutil.send_formatted_message(self.bot.makusu, commandutil.get_formatted_traceback(caught_exception))
    async def on_error(self, ctx, caught_exception):
        await commandutil.send_formatted_message(self.bot.makusu, commandutil.get_formatted_traceback(caught_exeception))
